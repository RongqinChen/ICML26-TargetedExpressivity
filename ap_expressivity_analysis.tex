...
\begin{proof}
    ...
    First, observe that $\pi$ preserves the cohesive structure: a set $S \subseteq V(G)$ is $c$-cohesive in $G$ iff $\pi(S) \subseteq V(H)$ is $c$-cohesive in $H$. 
    ...
    Similarly, minimal separators in $G$ correspond to minimal separators in $H$: if $S$ separates vertices $u,v$ in $G$, then $\pi(S)$ separates $\pi(u),\pi(v)$ in $H$, and $S$ is minimal iff $\pi(S)$ is minimal.
